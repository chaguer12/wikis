...
<div id="searchResults" class="flex flex-row flex-wrap mx-auto">
    ...
    <div class="container mx-auto text-center p-8">
      ...
      <?php
              ...
              if(isset($_GET['cat_id'])){
                $result = $wikiOBJ->Get_wiki_cat($_GET['cat_id']);
              }else{
      ...
      >
        <div
          class="flex flex-col w-96 pb-4 mb-6 transition-all duration-150 bg-white rounded-lg shadow-lg hover:shadow-2xl"
        >
          <div class="md:flex-shrink-0">
            <img
            src="data:image/jpg;charset=utf8;base64,<?= base64_encode($article['image']); ?>"
              alt="Blog Cover"
              class="object-fill h-64 w-full rounded-lg rounded-b-none md:h-56"
            />
          </div>
          ...
          <div class="flex flex-wrap items-center h-24 px-4 py-1 text-center mx-auto">
          ...
          <hr class="border-gray-300 " />
          <p
            class="flex flex-row h-64 flex-wrap w-full px-4 py-2 overflow-hidden text-sm text-justify text-gray-700"
          >
          <?php echo substr($article['contenu'],0,500); ?>...
          </p>
          <hr class="border-gray-300" />
          <input hidden type="text" value="<?php echo $article['user_id'];?>">
